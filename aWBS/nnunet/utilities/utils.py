...
def create_lists_from_splitted_dataset_folder(folder: str, file_ending: str, identifiers: List[str] = None) -> List[
    ...
    for f in identifiers:
        ...
        list_of_lists.append([join(folder, i) for i in files if i.startswith(f)])
...
def get_filenames_of_train_images_and_targets(raw_dataset_folder: str, dataset_json: dict = None):
    ...
    if 'dataset' in dataset_json.keys():
        ...
        identifiers = get_identifiers_from_splitted_dataset_folder(join(raw_dataset_folder, 'imagesTr'), dataset_json['image_ending'])
        images = create_lists_from_splitted_dataset_folder(join(raw_dataset_folder, 'imagesTr'), dataset_json['image_ending'], identifiers)
