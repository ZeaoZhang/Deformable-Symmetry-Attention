...
class U_Network(nn.Module):
    def __init__(self, dim, enc_nf, dec_nf, bn=None, full_size=True):
        ...
        self.flow = conv_fn(dec_nf[-1], dim + 1, kernel_size=3, padding=1)
...
class SpatialTransformer(nn.Module):
    def __init__(self, size, mode='bilinear', func='stf'):
    ...
    def forward(self, src, flow):
        if self.func == 'rstf2d':
            # compute grid
            new_locs = torch.einsum('bijyx,bjyx->biyx', w, flow[:, 1:, :, :]) - flow[:, 1:, :, :] + self.grid
            ...
            new_locs = self.grid + torch.cat([y, x, z]) - flow[:, 2:, ...]  # b,3,c,h,w
        shape = flow.shape[2:]
